...
from stream_generation.gen_index import gen_index, gen_pep
...
def pep_t(filename):
    ...
    g = compute_g(a, a_j, mu, r, n, h, F_q)
    #verifier interpolates the values provided by the prover, checks it agrees with prover and returns a_j if so
    v_alpha = check_g(sketch, g, mu, F_q)
    print(v_alpha == alpha)
...
def compute_g(a, a_j, mu, r, n, h, F_q):
    ...
    for t in range(degree + 1):
        ...
        denominators, grid = precomp_denominators(h, F_q)
        row_basis = F_q(compute_basis(h, F_q, x, denominators, grid))
        col_basis = F_q(compute_basis(h, F_q, y, denominators, grid))
        #compute P(x,y)
        t_val = F_q(0)
...
def verifier_pep(filename):
    ...
    alpha = next(stream)
    # check with prover
    return (F_q, mu, r, sketch, n, h, a_j, alpha)
